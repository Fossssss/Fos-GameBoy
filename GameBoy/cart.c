...
static const char* NEW_LIC_CODE[0xA5] = {
    [0x00] = "None",
    [0x01] = "Nintendo R&D1",
    [0x08] = "Capcom",
...
static const char* OLD_LIC_CODE[0x100] = {
    ...
    [0x31] = "Nintendo",
    [0x32] = "Bandai",
    [0x33] = "Indicates that the New licensee code should be used instead.", // 特殊处理，表示使用新的授权方代码
    [0x34] = "Konami",
    [0x35] = "HectorSoft",
    [0x38] = "Capcom",
    [0x39] = "Banpresto",
...
const char* cart_lic_name() {
    if (ctx.header->lic_code == 0x33){
        if (ctx.header->new_lic_code <= 0xA4) {
            return NEW_LIC_CODE[ctx.header->new_lic_code];
        }
        return "UNKNOWN";
    }
    else {
...
bool cart_load(char* cart) {
    ...
    char safe_title[17];
    memcpy(safe_title, ctx.header->title, 16);
    safe_title[16] = '\0';
